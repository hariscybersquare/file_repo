...
def scanfiles():
    ...
    """
    numberoftimes = settings.SCANNING_NUMBER
    while True:
        numberoftimes += 1
        filedetails = []
        with os.scandir(settings.FOLDER_PATH) as it:
            for entry in it:
                if not entry.name.startswith('.') and entry.is_file():
                    filedetailobj = File(entry.name, entry.path).filedetailsObj
                    if filedetailobj is not None:
                        filedetails.append(filedetailobj)
        if len(filedetails) > 0:
            dbupdate = DbModel()
            dbupdate.update_database(tuple(filedetails))
        time.sleep(settings.TIME_INTERVAL)
        if(numberoftimes > settings.SCANNING_NUMBER):
            break
